...
def detect_municipalites_relevance(file_content, metadata):

    # Step 1: Create an Assistant
    my_assistant_object = client.beta.assistants.retrieve(
        "asst_MOOPG5I91fAHXILvM6TmEiR1"
    )
    ...
    while my_run.status in ["queued", "in_progress"]:
        ...
        if keep_retrieving_run.status == "completed":
            ...
            metadata["doc_info"]["ai_municipalities"] = gpt_answer
...
def main(file, metadata):
    ...
    # Check if the it is a "vorlage"
    if metadata["doc_info"]["affair_type"] == "Vorlage":
        # Detect municipalities relevance
        detect_municipalites_relevance(file_content, metadata)
